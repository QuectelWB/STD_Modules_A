...
block:
	...
	state = defer_bh(dev, skb, &dev->rxq, state);  // 在 complete 函数完成时调用 tasklet_bh 

	if (urb) {
		if (netif_running (dev->net) &&
		    !test_bit (EVENT_RX_HALT, &dev->flags) &&
		    state != unlink_start) {
			rx_submit (dev, urb, GFP_ATOMIC);  // rx_complete 接收数据
				     ...
				     struct net_device *net)
{
	...
	if (length % dev->maxpacket == 0) {
		printk("[%s]  len %d max : %d  1\n",__func__, length, dev->maxpacket);
		if (!(info->flags & FLAG_SEND_ZLP)) {
		printk("[%s]  len %d max : %d  2  flags: %d \n",__func__, length, dev->maxpacket, info->flags);
			if (!(info->flags & FLAG_MULTI_PACKET)) {
				length++;
		printk("[%s]  len %d max : %d  FLAG_MUTI_PACKET  \n",__func__, length, dev->maxpacket);
